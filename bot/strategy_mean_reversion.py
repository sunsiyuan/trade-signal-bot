...
def build_mean_reversion_signal(
    ...
    atr = max(tf.atr, 1e-6)
    rsi6 = tf.rsi6
    oi_change_pct = snap.deriv.oi_change_pct or 0.0

    cond_far_below_ma = price <= ma25 - atr_dev_mult * atr
    cond_oversold = rsi6 <= rsi_oversold
    cond_oi_flushing_out = oi_change_pct <= -min_oi_change_pct

    if cond_far_below_ma and cond_oversold and cond_oi_flushing_out:
        from .signal_engine import TradeSignal
        ...
        reason = (
            ...
            f"RSI6={rsi6:.1f} oversold, OI_change={oi_change_pct:.1f}% flushing out"
        )
    ...
    cond_oi_squeezing = oi_change_pct >= min_oi_change_pct

    if cond_far_above_ma and cond_overbought and cond_oi_squeezing:
        from .signal_engine import TradeSignal
        sl = price + sl_buffer_mult * atr
        tp1 = ma25
        tp2 = ma25 - 0.5 * atr
        ...
        reason = (
            ...
            f"RSI6={rsi6:.1f} overbought, OI_change={oi_change_pct:.1f}% squeezing"
        )
