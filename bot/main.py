...
def print_signal(signal):
    ...
    oi_change = (
        ...
        if d.oi_change_24h and abs(d.oi_change_24h) >= 0.01
        else ""
    )
    print(
...
def format_notification(signal, threshold: float = 0.8):
    ...
    def _oi_line():
        oi_value = f"{snap.deriv.open_interest:.2f}"
        change = snap.deriv.oi_change_24h
        state = _oi_state()

        if change is None:
            return f"• OI: {oi_value}"
        ...
        if state:
            return f"• OI: {oi_value} → {state}{change_text}"
        return f"• OI: {oi_value}{change_text}"
